...
2. Summarize the printed report honestly: it covers the **local** `usage.jsonl` only. Rows with `cost_usd` are summed as dollars. Rows with only a tier use labeled relative-unit estimates. It is **not** live Cursor/Claude/Codex billing, and it does not read a price table unless you pass one to the estimator.
