#!/usr/bin/env python3
"""Shared router for hosts and optional CodeFriends agents.

One policy, one savings log. CodeFriends does not have to import this.
Call ``route`` after the user confirms; spendy tiers stay blocked until then.

    from auto_model_router import route

    result = route(
        task,
        confirmed=True,  # bool True only after an explicit user confirm
    ...
    if not result["allowed"]:
        raise SystemExit(result["suggestion"])  # do not run tools
"""
...
# Illustrative only. Used when a log row has a tier and no real cost or priced tokens.
...
def classify_task(task: str) -> dict:
    """Classify with confidence. Vague low-confidence prompts are standard."""
...
def evaluate_tool(
    ...
    """Block spendy tool execution unless the gate file says the user confirmed.
    ...
    elif tier in SPENDY_TIERS and confirmed:
        allowed = True
        reason = "explicit user confirm is on file for this spendy tier"
    else:
        allowed = False
        if forged:
            reason = "blocked: tool arguments cannot self-confirm a spendy run"
        elif tier in SPENDY_TIERS:
...
def route(
    ...
    *,
    ...
    Spendy tiers return ``allowed`` False until one of those is set.
    ...
    decision = evaluate_tool({"tier": chosen, "confirmed": user_confirmed}, None)
    record: Dict[str, Any] = {
        "tier": chosen,
        "suggested_tier": suggested,
        "reason": classified["reason"],
        "confidence": classified["confidence"],
        "needs_confirm": chosen != "fast" and not user_confirmed,
    ...
    if state_path is not None:
        save_gate(
            {
                "tier": chosen,
                "confirmed": user_confirmed or chosen == "fast",
                "overridden": overridden,
                "suggested_tier": suggested,
                "confidence": classified["confidence"],
                "source": "route",
...
def summarize_usage(
    ...
    return {
        ...
        "notes": notes,
        # Compatibility keys when every row is illustrative.
