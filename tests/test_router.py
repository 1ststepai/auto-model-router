#!/usr/bin/env python3
"""Gate, confidence downshift, usage log, and CodeFriends route contract."""
...
from demo.classify import EXAMPLES, LOW_CONFIDENCE, classify  # noqa: E402
...
class ConfidenceTests(unittest.TestCase):
    def test_examples_keep_their_tiers(self) -> None:
        for task, expected in EXAMPLES:
            self.assertEqual(classify(task)["tier"], expected, task)
...
class UsageTests(unittest.TestCase):
    ...
    def test_filled_price_table_prices_tokens(self) -> None:
        table = {
            "currency": "USD",
            "tiers": {
                tier: {"input_per_million": 1.0, "output_per_million": 2.0}
                for tier in ("fast", "standard", "reasoning", "max")
            },
        }
        summary = summarize_usage(
            [{
...
class CodeFriendsTests(unittest.TestCase):
    def test_route_contract_blocks_then_logs(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            blocked = route(task, confirmed=False, log_path=log, host="codefriends")
            ...
            self.assertEqual(row["host"], "codefriends")
